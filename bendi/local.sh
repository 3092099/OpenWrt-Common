...
wget -O common.sh https://raw.githubusercontent.com/3092099/OpenWrt-Common/main/common.sh
if [[ $? -ne 0 ]]; then
  curl -fsSL https://raw.githubusercontent.com/3092099/OpenWrt-Common/main/common.sh > common.sh
fi
...
if [[ ! -f "operates/${FOLDER_NAME}/settings.ini" ]]; then
  ...
  curl -fsSL https://raw.githubusercontent.com/3092099/OpenWrt-Common/main/bendi/tongbu.sh -o tongbu.sh
...
git clone -b main --depth 1 https://github.com/3092099/OpenWrt-Common build/common
judge "扩展文件下载"
  ...
  if [[ -d "amlogic" ]]; then
    ...
    if [[ "${HOUR}" -lt "12" ]]; then
      ...
      if [[ -d "amlogic" ]]; then
        ...
        if [[ $? -ne 0 ]]; then
          curl -fsSL https://github.com/3092099/OpenWrt-Common/releases/download/API/stable.api -o amlogic/stable.api
        fi
